...
contract OmicronTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SOMIC }
    ...
    address public sOMIC;
    uint public sOMICQueue; // Delays change to sOMIC address
    ...
    /**
        @notice allow approved address to deposit an asset for OMIC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint OMIC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( OMIC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn OMIC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOMICERC20( OMIC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sOMIC ).balanceOf( msg.sender ); // Can only borrow against sOMIC held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with OMIC
        @param _amount uint
     */
    function repayDebtWithOMIC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IOMICERC20( OMIC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, OMIC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( OMIC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( OMIC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns OMIC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match OMIC decimals
            value_ = _amount.mul( 10 ** IERC20( OMIC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SOMIC ) { // 9
            sOMICQueue = block.number.add( blocksNeededForQueue );
        } else return false;
