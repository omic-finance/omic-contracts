...
contract Z2OBondDepository is Ownable {
    ...
    address public immutable Z2O; // token given as payment for bond
    ...
    function updateDAO(address newDao) public onlyAdmin {
        DAO = newDao;
    }
    ...
    function deposit( 
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( Z2O ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( Z2O ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( Z2O ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( Z2O ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {  
        uint supply = IERC20( Z2O ).totalSupply();
